...
import { GameEvent, PlayerEvent, CometEvent } from '../../shared/events.model';
import { LoginScene } from "../scenes/login.class";
import { Window, SpaceShip, Coordinates, Comet, Player as PlayerType } from '../../shared/models';
...
export class Game {
    ...
    protected manageAssets(game: PhaserGame): void {
        ...
        window.socket.on(GameEvent.drop, (coords: Coordinates)=>{
            if(this.projectile){
                this.projectile.pickup.item.kill();
            }

            this.projectile = new Projectile(game);
            this.projectile.renderPickup(coords);
        });
